...
class BranchController extends Controller
{
    ...
    public function index(){
        ...
        return view("branch.list",compact('advanceSearchLabel','branchSearch','statesPH','states','statesCN','upline'));
    }
    ...
    public function branch_data()
    {
        ...
        $access = session('all_user_access');
        $admin_access = isset($access['admin']) ? $access['admin'] : "";
        $branchaccess = isset($access['branch']) ? $access['branch'] : "";
        ...
        $pt_id="";
        if (Access::hasPageAccess('branch','view',true)){
            $result = Partner::get_partners($partner_access,9,$id, -1, -1,"","","'A','I','T','V','P','C'");
            foreach ($result as $p) {
                $unverified="";
                ...
                $upline = Partner::find($p->parent_id);
                $incomplete = "";
                if($p->federal_tax_id == "" || $p->merchant_mid == "" || $p->credit_card_reference_id == ""
                    ...
                    $incomplete = ' <span title="Incomplete Branch Info"><i class="fa fa-exclamation-triangle big-icon"></i></span> ';
                }

                $linkOpening = "<a href='/merchants/branchDetails/{$p->partner_id}/profile'>";
                $linkClosing = "</a>";
                ...
                $out_merchants[] = array(
                    $p->partner_id_reference,
                     "<a href='/merchants/details/{$upline->id}/profile'>". $upline->partner_company->company_name . "</a>",
    ...
    public function merchant_branch_data($id)
    {
        $partner_access=-1;
        $access = session('all_user_access');
        $admin_access = isset($access['admin']) ? $access['admin'] : "";
    ...
    {
            $query = Partner::where('partner_type_id', 9)->with('partner_company');
            ...
            if (Access::hasPageAccess('merchant','view',true)){
                foreach ($query->get() as $p) {
                    ...
                    $upline = Partner::find($p->parent_id);
                    $view='<a class="btn btn-primary btn-sm" href="/merchants/branchDetails/'.$p->id.'/profile">View</a>';
                    ...
                    $linkOpening = "<a href='/merchants/branchDetails/{$p->id}/profile'>";
                    $linkClosing = "</a>";
    ...
    public function create(){
        ...
        return view('branch.create', compact('formUrl', 'statePH', 'stateUS', 
            'stateCN', 'country', 'ownership', 'documents', 'canSaveAsDraft', 
            'bankAccountType', 'systemUser','userDepartment','upline','is_internal',
            'language','ownerships','systems','businessTypeGroups','initialCities'));
    }  

    public function confirmBranch($id) {
        ...
        return Array('success' => true,'message' => 'Branch has been boarded');
    }

    public function approveBranch(){
        $advanceSearchLabel = 'Branch';
        $branchSearch = true;
        $statesPH = State::where('country','PH')->orderBy('name')->get();
        $states = State::where('country','US')->orderBy('name')->get();
        $statesCN = State::where('country','CN')->orderBy('name')->get();
        $access = session('all_user_access');
        return view("branch.approve",compact('advanceSearchLabel','branchSearch','statesPH','states','statesCN'));
    } 

    public function finalizeBranch($id) {
        ...
        return Array('success' => true,'message' => 'Branch has been approved');
    }

    public function declineBranch(Request $request, $id)
    {
        if (Access::hasPageAccess('branch', 'decline branch', true)) {
            $partner = Partner::find($id);
            ...
            return [
                'success' => true,
                'message' => 'Branch has been declined'
            ];
        }
    ...
    public function draftBranch(){
        ...
        $access = session('all_user_access');
        $merchantaccess = isset($access['branch']) ? $access['branch'] : "";
        $merchantaccess1 = isset($access['merchant']) ? $access['merchant'] : "";
        $canViewUpline = (strpos($merchantaccess1, 'view upline') === false) ? false : true;
        return view("branch.draft",compact('advanceSearchLabel','merchantSearch','statesPH','states','statesCN','canViewUpline'));
    } 

    public function branch_draft_data()
    {
        $partner_access=-1;
        $id = auth()->user()->reference_id; 
        ...
        if (Access::hasPageAccess('draft applicants','draft applicants list',true)){
            $result = DraftPartner::get_draft_partners($partner_access,9,$id, -1, -1);
            foreach ($result as $p) {
                $unverified="";
                $upline = Partner::find($p->parent_id);
                $incomplete = "";
                ...
                $linkOpening = "<a href='/drafts/draftBranch/" . $p->partner_id . "/" . $p->partner_type_id . "/edit'>";
                $linkClosing = "</a>";
                $view = '<button class="btn btn-danger btn-sm" onclick="deleteDraftBranchApplicant(' . $p->partner_id . ')" title="Delete"><i class="fa fa-trash"></i></button>';
                $status = '<span style="color:orange">Incomplete Branch Application</span>';
    ...
    public function dashboard($id){
        $merchant = Partner::where('id',$id)->where('partner_type_id',9)->first();
        $partner_access=-1;
        $access = session('all_user_access');
        ...
        return view("branch.details.dashboard",compact('js','merchant','id'));    
    }
    ...
    private function buildTree($partners,$isPartner){
        ...
        foreach($partners->sortBy('level') as $key => $treeInfo){
            ...
            elseif($treeInfo->partner_type->name == 'BRANCH'){
                $edit = 'desc: { 
                            val: " ",
                            href: "/merchants/branchDetails/profile/'.$treeInfo->id.'/profileCompanyInfo",
                            target: "_self"
                        },';                
            }
            else{
    ...
    public function profile($id){
    	...
    	if(!isset($merchant))
    	{
    		return redirect('merchant/branch')->with('failed','Cannot find branch');
    	}
    	$country = Country::get();
    	$ownership = Ownership::get();
    	$statePH = State::where('country','PH')->orderBy('name')->get();
        ...
        if ($partner_access==""){$partner_access=$id;}
    	$upline_partners =  Partner::get_upline_partner($id,$partner_access,3);  
        ...
        return view("branch.details.profile",
            compact('id','merchant','country','ownership','statePH','stateUS',
                ...
                'comments','bankAccountType','language','systems','isInternal',
                'businessTypeGroups','states','initialCities'));
    }
    ...
    public function products($id){
        ...
        if (!$canListOrder && !$canCreateOrder){
            return redirect('/merchants/branchDetails/'.$id.'/profile')->with('failed','No access to this page');
        }

    	$merchant = Partner::with('partner_company')->where('id',$id)->where('partner_type_id',9)->first();
        $parent_merchant = Partner::find($merchant->parent_id);
    	if($merchant->parent_id == -1){
            $partner_product_id = -1;  
    	}else{
    		$product_id="";
    		$products = PartnerProduct::get_partner_products($parent_merchant->parent_id);
		...
		{
            $p->subproducts = Product::get_child_products($p->id,$parent_merchant->parent_id);
            $categories = Array();
        ...
        return view("branch.details.products",compact('id','merchant','products','formOrderUrl','orders','formOrderEditUrl','has_products','canSign','canListOrder','canCreateOrder','canProcessOrder','canEditPaymentFrequency','active_tab','payment_types'));
    }
    ...
    public function billing($id){
        ...
        if ($partner_id==$id){
            $partner_info = Partner::get_partner_info($id,false,"9");
        } else {
            $partner_info = Partner::get_partner_info($id,false,"9",$partner_access);    
        }
        ...
        return view("branch.details.billing",compact('id','merchant','partner_info','payment_types','payment_methods','invoices','recurring','email','canPay','canVoid','canCharge','canCreate'));
    }
    ...
    public function storeBranch(Request $request)
    {
    	DB::transaction(function() use ($request, &$id, &$user){
	    	$partner = new Partner;
	    	$partner->partner_type_id = 9; 
	    	$partner->original_partner_type_id = 9;
            ...
            $partner_id_reference =  "B" . (100000+$max_count); 
            ...
            $partnerCompany->phone2 = $request->txtPhoneNumber2;
            ...
            if (isset($request->copy_to_billing)) {
                $partnerBill->country = $request->txtCountry; 
                $partnerBill->address = $request->txtAddress;
                $partnerBill->address2 = $request->txtAddress2;
                $partnerBill->city = $request->txtCity;
                $partnerBill->state = $state;
                $partnerBill->zip = $request->txtZip;	
            } else {
                $partnerBill->country = $request->txtBillingCountry;
                $partnerBill->address = $request->txtBillingAddress;
                $partnerBill->address2 = $request->txtBillingAddress2;
                $partnerBill->city = $request->txtBillingCity;
                $partnerBill->state =  $bill_state;
        ...
        $access = session('all_user_access');
        $merchantaccess = isset($access['merchant']) ? $access['merchant'] : "";
        return redirect('/merchants/branchDetails/'.$id.'/products?tab=create-order')->with([
    ...
    public function updateBranchInfo($id,Request $request)
    {
    	DB::transaction(function() use ($id,$request){
	    	$partner = Partner::find($id);
	    	$partner->parent_id = $request->txtUplineId == null ? $partner->parent_id : $request->txtUplineId;
	    	$partner->merchant_processor = $request->txtProcessor; 
            ...
            if($partner->merchant_status_id == MerchantStatus::LIVE_ID){
    	...
    	return redirect('/merchants/branchDetails/'.$id.'/profile#businfo')->with('success','Branch Updated');

    }

	public function updateBranchAddress($id,Request $request) {
    	...
    	DB::transaction(function() use ($request, &$id, &$user){
	    	...
	    	/* $partnerDBA = PartnerDbaAddress::where('partner_id',$id)->first();
		    	$partnerDBA->country = $request->txtCountry;
		    	$partnerDBA->address =  $request->txtAddress;
		    	$partnerDBA->address2 =  $request->txtAddress2;
		    	$partnerDBA->city = isset($request->txtCity) ? $request->txtCity : $partnerDBA->city;
		    	$partnerDBA->state = $state;
		    	$partnerDBA->zip = isset($request->txtZip) ? $request->txtZip : $partnerDBA->zip;
	    	$partnerDBA->create_by = auth()->user()->username;
	    	$partnerDBA->update_by = auth()->user()->username;
	    	$partnerDBA->save(); */
	    	...
	    	/* $partnerShip = PartnerShippingAddress::where('partner_id',$id)->first();
		    	...
		    	$partnerShip->address = $request->txtShippingAddress;
		    	$partnerShip->city = $request->txtShippingCity;
		    	$partnerShip->state = $request->txtShippingState;
        ...
        return redirect('/merchants/branchDetails/'.$id.'/profile#adrs')->with([
    ...
    public function branch_contact_info($id)
    {
	...
	public function updateBranchContact($id,Request $request){
    	DB::transaction(function() use ($id, $request){
            $access = session('all_user_access');
            $merchantaccess = isset($access['branch']) ? $access['branch'] : "";
            ...
            if($request->isOrigCon == 1){
                ...
                if(isset($user)){
                    ...
                    return redirect('/merchants/branchDetails/'.$id.'/profile#ownrinf')->with([
    	...
    	return redirect('/merchants/branchDetails/'.$id.'/profile#ownrinf')->with('success','Contacts Updated');
    }

	public function uploadBranchAttachment(Request $request){
    	...
    	return redirect('/merchants/branchDetails/'.$id.'/profile#atch')->with('success','Attachments Updated');
    }
	...
	public function updateBranchPaymentgateway($id,Request $request){
    	DB::transaction(function() use ($id,$request){
    		if($request->pgID == -1)
    		{
    	...
    	return redirect('/merchants/branchDetails/'.$id.'/profile#paygate')->with('success','Payment Gateway Updated');
    }
    public function addComment(Request $request){
        ...
        return redirect('/merchants/branchDetails/'.$request->txtPartnerId.'/profile#dis')->with('success','Comment has been posted!');
    }
    public function addSubComment(Request $request){
        ...
        return redirect('/merchants/branchDetails/'.$request->txtPartnerId1.'/profile#dis')->with('success','Sub Comment has been posted!');
    }
    ...
    public function create_order($id,Request $request)
    {
        ...
        return DB::transaction(function() use ($id,$request, $involvedStatuses) {
            ...
            $merchant = Partner::where('id',$order->partner_id)->where('partner_type_id',3)->first();
    ...
    public function advance_branch_search($country, $state,$status){
        $partner_access=-1;
        $id = auth()->user()->reference_id; 
        ...
        $out_merchants=array();
        if (Access::hasPageAccess('branch','view',true)){
            $result = Partner::get_partners($partner_access,9,$id, -1, -1,"",$search,"'A','I','T','V','P','C'");
            foreach ($result as $p) {
                $unverified="";
                $verify_mobile = Country::where('country_calling_code',$p->country_code)->first()->validate_number;
                if($verify_mobile==1)
  ...
  //       ->make(true);
    ...
    public function workflow($id,$order_id)
    {
    	...
    	if ($noTemplate) {
    		return redirect('/merchants/branchDetails/'.$id.'/products#history')->with('failed','There is no template defined for this product!');
    	}
        ...
        return view("branch.workflow.workFlow")->with(
            compact(
                'id',
                'merchant',
                'headername',
    ...
    public function markAllTaskAsCompleted(Request $request, $partnerId, $orderId)
    {
        ...
        return redirect("/merchants/branchWorkflow/{$partnerId}/{$orderId}");
    }
        ...
        $result = DB::transaction(function() use ($id){
            try{
                ...
                if($hasCC){
                    ...
                    if( $msg != 'success'){
                        DB::rollback();
                        return Array('message' => 'Error Processing Cardpoint Order!');
                    }
                }
    ...
    public function updateRecurring(Request $request){
        ...
        return redirect('/merchants/branchDetails/'.$request->txtFrequencyPID.'/billing#recInv')->with('success','Recurring Invoice Updated');
    }
    ...
    public function confirmPage($id){
        ...
        if (auth()->user()->id !== $partner_id) {
            $confirmationMessage = "I, {$merchant->partnerContact->first_name} {$merchant->partnerContact->last_name} ";
    ...
    public function invoices_management() {
        ...
        $merchant = Partner::where('id',$partner_id)->where('partner_type_id',3)->first();
    ...
    public function invoices()
    {
        ...
        return view("merchants.details.allinvoices",compact('payment_types','canPay','canVoid','canCharge','canCreate'));
    }
    ...
    public function invoices_data()
    {
        ...
        foreach($invoices as $invoice)
        {
            ...
            $view = '<a class="btn btn-default btn-sm" href="javascript:void(0);" onclick="showInvoice('.$invoice->id.')">View</a>';

            $out_invoices[] = array(
                $invoice->id,
                $invoice->invoice_date,
                $invoice->partner->partner_company_parent->company_name,
    ...
    public function workflow_data() {
        ...
        foreach($tasks as $task)
        {
            ...
            {
                $view = '<a class="btn btn-default btn-sm" href="\merchants\workflow\\'.$task->productOrder->partner_id.'\\'.$task->productOrder->id.'">View</a>';
                $out_tasks[] = array(
                    $task->id,
                    $task->created_at->format('m/d/Y'),
                    $task->productOrder->partner->partner_company_parent->company_name,
                ...
                if(!empty($data) && $data->count()){
                    ...
                    foreach ($data as $key => $value) {
                        ...
                        if ($value->partner_type == '' || strtolower($value->partner_type) != 'merchant') {
                            ...
                            $insert_partner_companies->country              = trim($value->country);
                            ...
                            $insert_partner_contacts->country                = trim($value->country);
