...
class LeadsProspectsController extends Controller
{
    public function __construct()
    {
        $this->middleware('access:lead,view')->only('index');
        $this->middleware('access:lead,add')->only('create');
    }
    ...
    public function index(){
        ...
        $leads = Partner::partner_leads_prospects($partner_access,6);
        $draftLeads = DraftPartner::get_draft_leads_prospects($partner_access,6);
        $leads = array_merge($leads, $draftLeads);
        
        foreach ($leads as $l) {
        $interested_products = '';  
            foreach ($l->interested_products as $i) {
                $interested_products .= $i->name .'; ';
                $l->interested_products = $interested_products;
            }
        }
        foreach ($leads as $l) {
        $upline = '';
            foreach ($l->upline_partners as $u) {
                //$upline .= $u->first_name .' '. $u->last_name .' - '. $u->merchant_id;
                $upline .= $u->company_name .' - '. $u->merchant_id.'<br>';
                $l->upline_partners = $upline;
            }
        }
        ...
        $leadaccess = isset($access['lead']) ? $access['lead'] : "";
        $canViewUpline = (strpos($leadaccess, 'view upline') === false) ? false : true;

        return view("leads.list", compact('leads','advanceSearchLabel','leadsSearch','products','canDelete','canViewUpline'));
    }
    public function create(){
        ...
        return view('leads.create', compact('partner_type', 'upline_partner_type',
    ...
    public function profile($partner_id=null){
        ...
        $canConvert = 0;
        if (Access::hasPageAccess('lead','convert',true) || Access::hasPageAccess('prospect','convert',true)) {
            $canConvert = 1;
        }
        ...
        $canAdd = 0;
        if (Access::hasPageAccess('lead','add',true) || Access::hasPageAccess('prospect','add',true)) {
            $canAdd = 1;
        }
        ...
        if (!$isInternal) {
            return redirect("/leads/details/summary/{$partner_id}");
        }
        ...
        return view("leads.details.profile", 
            compact('partner_id','partner_info','partner_type','country','state','statePH','stateCN',
            'ownership','canEdit','canConvert','upline','is_admin','canAdd','partner_status','comments',
            'calling_code','assigned_id', 'businessTypeGroups','paymentProcessor','leadStatus','isInternal',
    ...
    public function contact($partner_id=null)
    {
        ...
        return view("leads.details.contact", 
    ...
    public function interested($partner_id=null)
    {
        ...
        if (strpos($admin_access, 'super admin access') === false && $partner_info[0]->parent_id == -1) {
            ...
            if ($parent_id == -1) {
                ...
                $partner_product = DB::table('partner_product_accesses')->where('partner_id',$parent_id)->first();
                $partner_product_id = "";
                if(isset($partner_product->product_access)) $partner_product_id = $partner_product->product_access;
                if ($partner_product_id == '') {
                    $partner_product_id = -1;
                }
                if ($partner_product_id != -1 && $partner_product_id != ""){
        ...
        return view("leads.details.interested", 
    ...
    public function application($partner_id=null)
    {
        ...
        return view("leads.details.products", 
            compact('partner_id','partner_info','products','calling_code','isInternal'));
    }
    public function appointment($partner_id=null){
        ...
        return view("leads.details.appointment", 
    ...
    public function incoming(){
        ...
        $incoming_leads = IncomingLead::get_incoming_leads($id,6);

        return view("leads.incoming",compact('incoming_leads')); 
    }
    public function createLeadProspect(Request $request){  
        if(Input::get('assignTo') && !Input::get('assignee')){
            ...
            $pc1_exist = DB::select(DB::raw("SELECT pc.id FROM partner_companies pc
                ...
                WHERE p.status = 'A' AND UCASE(pc.email) = '".Input::get('txtEmailLead')."'"));
            ...
            */
            ...
            DB::transaction(function() use ($request) {
                ...
                $partnerCompany->email = Input::get('txtEmailLead');
                ...
                $partnerContact->email = Input::get('txtEmail2Lead');
                ...
                if($original_parent_id != $creator_id){
                    ...
                    if (Input::get('txtEmailLead')) {
                        $data = array(
                            'first_name' => Input::get('fname'),
                            'last_name' => Input::get('lname'),
                            'email_message' => $email_message,
                            'email_address' => Input::get('txtEmailLead'),
                        ...
                        if (Mail::failures()) {
                            return redirect('/leads')->with('failed','Failed to send email.');
                        }  
                    }
                }
            });
    ...
    public function updateIncomingLeadRequest(Request $request){
        ...
        if(!$updateRequest->save()){
            return response()->json(array(
                'success'                   => false, 
                'msg'                       => "Unable to update incoming leads!", 
            ), 200);
        }
        ...
        $email_message = $info[0]->assignee." Lead request has been " . $status_text;
        ...
        for ($i=0; $i < sizeof($recipients); $i++) { 
            ...
            $notifications->status = 'N';
            $notifications->create_by = auth()->user()->username;
            $notifications->redirect_url = '/leads/details/profile/'.$info[0]->partner_id;
            $notifications->save();
    ...
    public function updateLeadProspect(){
        ...
        if (Input::get('assignedID') != Input::get('assignTo') && Input::get('assignTo') == -1) {
            ...
            if (isset($check_ic->id)) {
                return response()->json(array(
                    'success'   => false, 
                    'msg'       => "There is a current request for this lead. You are not allowed to change the assigned person to 'Unassigned' until it has been declined, accepted or the request expires.", 
                ), 200);
            }
        }
        if ($parent_id != $previous_parent_id && $parent_id !=$assigned_id) { 
            if ($parent_id != -1) {
                ...
                if (isset($check_ic->id)) {
                    return response()->json(array(
                        'success'   => false, 
                        'msg'       => "There is a current request for this lead. You are not allowed to change the assigned person until it has been declined, accepted or the request expires.", 
                    ), 200);
                }
            }
            ...
            if (!$incoming_leads->save()) {
                return response()->json(array(
                    'success'   => false, 
                    'msg'       => "Unable to create incoming leads request", 
                ), 200);
            }
    ...
    public function convertToMerchant(){
        ...
        if (!$convertToMerchant->save()) {
            return response()->json(array(
                'success'   => false, 
                'msg'       => "Unable to convert lead.", 
            ), 200);
        }else {
            return response()->json(array(
                'success'   => true, 
                'msg'       => "Lead has been successfully converted to merchant.", 
            ), 200);
        }
    }
            ...
            if ($extension == "csv") {
                ...
                if(!empty($data) && $data->count()){
                    ...
                    foreach ($data as $key => $value) {
                        $skip = false;
                        if ($value->partner_type == '' || strtolower($value->partner_type) != 'lead') {
        ...
        if (strpos($admin_access, 'super admin access') !== false){
            return $dt::of($result)
                                ->editColumn('merchant_id', function ($result) {
                                    $mid='<a href="/leads/details/profile/'.$result->partner_id.'">'.$result->merchant_id.'</a>';
                                    return $mid;
                                })
                                ...
                                /* ->editColumn('action', function($result){
                                    $deleteLead = '<button type="button" class="btn btn-danger btn-sm" onclick="deleteLead('.$result->partner_id.',\'D\')">Delete</button>';
                                    return $deleteLead; 
                                }) */
    ...
    public function deleteLead(){
        DB::transaction(function(){
            ...
            if(!$updatePartner->save()){
                return response()->json(array('success' => false, 'msg' => 'Unable to remove lead.'), 200);
            }
        });
        return response()->json(array('success' => true, 'msg' => 'Lead successfully removed!'), 200);
    }
    public function convertToProspect(){
        $convertToMerchant = Partner::find(Input::get('txtLeadID'));
        $new_pir = "P" . $convertToMerchant->partner_id_reference;
        $convertToMerchant->original_partner_id_reference = $convertToMerchant->partner_id_reference;
        $convertToMerchant->original_partner_type_id = $convertToMerchant->partner_type_id;
        $convertToMerchant->conversion_date = date('Y-m-d H:i:s');
        ...
        $convertToMerchant->partner_id_reference = $new_pir;
        $convertToMerchant->status = 'A';
        $convertToMerchant->partner_type_id = 8;
        ...
        if (!$convertToMerchant->save()) {
            ...
            return response()->json(array(
                'success'   => true, 
                'msg'       => "Lead has been successfully converted to prospect.", 
            ), 200);
        }
    }
    ...
    public function summary($partner_id = null) {
        ...
        return view("leads.details.summary", 
            compact('partner_id','partner_info','partner_type','country','state',
            'statePH','stateCN','ownership','canEdit','canConvert','upline',
            'is_admin','canAdd','partner_status','comments','calling_code',
            'assigned_id', 'businessTypeGroups','isInternal','states'));
    }

    public function getAppointments(Datatables $dt,$partner_id) {
        ...
        $tasks = DB::select(DB::raw("select ca.*,ca2.attendees as parent_attendees,CONCAT(u.first_name,' ',u.last_name) as organizer,
            ...
            left join partners p on p.id = ca.partner_id
            left join partner_contacts pc on pc.partner_id = ca.partner_id
            where ca.status not in ('D','I','CF','DC') and ca.partner_id = " .$partner_id." order by start_date desc"));
            // where ca.status not in ('D','I','CF','DC')"));
            return $dt::of($tasks)
                                ...
                                ->editColumn('start_date', function($tasks){
                                    return date_format(date_create($tasks->start_date),"m/d/Y h:i:s A");
                                })
                                ->editColumn('end_date', function($tasks){
                                    return date_format(date_create($tasks->end_date),"m/d/Y h:i:s A");
                                })
                                ->editColumn('title', function ($tasks) {
                                    return '<a href="javascript:void(0);" onclick="editAppointment('.$tasks->id.',\''.date_format(date_create($tasks->end_date),"Y-m-d").'\')"><i class="fa fa-pencil"></i></a>&nbsp;&nbsp;&nbsp;'.$tasks->title;
                                })
                                ->rawColumns(['title'])
                                ->make(true);
