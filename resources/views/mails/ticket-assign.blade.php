...
<body class="body">
    <div class="inside-content">
        <table>
            <tr>
                <td>
                    <h3 class="space" style="display: initial;">Ticket #{{ $ticketHeader->id }} is assigned to {{ $ticketHeader->assignee == -1 ? "{$ticketHeader->userType->description} Department" : $ticketHeader->assignedTo->first_name . ' ' . $ticketHeader->assignedTo->last_name }}   </h3><br>
                    <p class="color-disable" style="margin: 5px 5px 5px 10px; border-bottom: dotted 2px;line-height: 20px;">{{ \Carbon\Carbon::parse($ticketHeader->updated_at)->format('F d, Y H:i:s A T') }}</p>
                </td>
            </tr>
        </table>
        <div class="message">
    ...
    <table>
        ...
        <tr>
            ...
            <td class="color-disable space">{{ $ticketHeader->userType->description }}</td>
        </tr>
        <tr>
