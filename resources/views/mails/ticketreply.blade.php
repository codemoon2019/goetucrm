  ...
  <div class="inside-content">
    <table>
      <tr>
        ...
        <td>
          <h3 class="space" style="display: initial;">{{$detail['first_name']}} {{$detail['last_name']}}</h3><br>
          <p class="color-disable" style="margin: 5px 5px 5px 10px; border-bottom: dotted 2px;line-height: 20px;">{{$detail['datetime']}}</p>
        </td>
      </tr>
    </table>
    <div class="message">
      <p>{!! html_entity_decode($detail['message']) !!}</p>
    </div>
  </div>
  @endforeach
  <div class="inside-content">
    <table>
      <tr>
        ...
        <td>
          <h3 class="space" style="display: initial;">{{$header['first_name']}} {{$header['last_name']}}
          @if($header['care_of']!="") 
              <span style="font-size: 12px;" class="color-disable space">via {{$header['care_of']}}</span> 
            @endif
          </h3><br>
          <p class="color-disable" style="margin: 5px 5px 5px 10px; border-bottom: dotted 2px;line-height: 20px;">{{$header['datetime']}}</p>
        </td>
      </tr>
    </table>
    <div class="message">
      <p>{!! html_entity_decode($header['message']) !!}</p>
    </div>
  </div>
  <hr>
