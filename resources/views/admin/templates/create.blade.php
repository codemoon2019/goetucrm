            ...
            <div class="nav-tabs-custom">
                ...
                <div class="tab-content no-padding">
                    <div class="tab-pane active">
                        <form role="form" action="{{ url("/admin/group-templates") }}"  enctype="multipart/form-data" method="POST">
                        {{ csrf_field() }}
                        <input type = "hidden" id="access" name="access"/>            
                        <div class="row">
                            <div class=" col-md-6 form-group">
                                <label>Name:</label>
                                <input type="text" id="name" name="name" class="form-control dept-acl-input"  placeholder="Template Name..." value="">
                            </div>
                        ...
                        <tr id="sub-{{$acl['id']}}" style="display: none;" class="sub-tr">
                            ...
                            <div class="row">
                               ...
                               <div class="col-sm-4">
                                 <input data-toggle="tooltip" data-html="true" title="{!! html_entity_decode($value->description)!!}" type="checkbox" name="{{$value->name}}" id="{{$value->name}}" value="{{$value->id}}" class="acl-cb" /> <label>{{str_replace('Add','Create',$value->name)}}</label>  
                               </div>
                               @endforeach   
                            </div>  
                            </td>
