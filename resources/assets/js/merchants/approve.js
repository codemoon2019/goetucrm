...
$(document).ready(function () {
    ...
    $(document).on('click', '#searchMerchants', function (e) {
        ...
        $.getJSON('/merchants/search/'+filterType+'/'+txtSearchValue+ '/C', null, function(data) {  
    ...
    $(document).on('click', '#searchBranches', function (e) {
        ...
        $.getJSON('/merchants/branchSearch/'+filterType+'/'+txtSearchValue+ '/C', null, function(data) {  
            var oTable = $('#merchant-list').dataTable( {"bRetrieve": true} );
            oTable.fnClearTable();
            if (data.length >0){
                oTable.fnAddData(data);    
...
function advanceSearchMerchants() {
    ...
    $.getJSON('/merchants/advance_merchants_search/' + country + '/' + states+ '/C', null, function(data) {  
        var oTable = $('#merchant-list').dataTable( {"bRetrieve": true} );
        oTable.fnClearTable();
        if (data.length >0){
            oTable.fnAddData(data);    
...
function load_merchants(){ 
    $.getJSON('/merchants/merchant_approve_data', null, function(data) {  
        var oTable = $('#merchant-list').dataTable( {"bRetrieve": true} );
        oTable.fnClearTable();
        if (data.length >0){
            oTable.fnAddData(data);    
...
function load_branches(){ 
    $.getJSON('/merchants/branch_approve_data', null, function(data) {  
        var oTable = $('#merchant-list').dataTable( {"bRetrieve": true} );
        oTable.fnClearTable();
        if (data.length >0){
            oTable.fnAddData(data);    
...
function advanceSearchBranches() {
    ...
    $.getJSON('/merchants/advance_branch_search/' + country + '/' + states + '/C', null, function(data) {  
        var oTable = $('#merchant-list').dataTable( {"bRetrieve": true} );
        oTable.fnClearTable();
        if (data.length >0){
            oTable.fnAddData(data);    
