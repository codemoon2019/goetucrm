...
$(document).ready(function () {
    ...
    // Replicated from MerchantProductOrder
    ...
    //     $.getJSON('/prospects/select_payment_frequencies', null, function (data) {
    ...
    if ($('#countryName').val()) {
        ...
        $.ajax({
            type: 'GET',
            url: '/prospects/getCountryCallingCode',
            data: data,
            dataType: 'json',
    ...
    $('#saveLeadProspect').click(function () {
        ...
        if ($('#txtEmailPros').val() != "") {
            if (!isEmail($('#txtEmailPros').val())) {
                alert('Invalid email format');
                return false;
            }
        }
        // if ($('#txtEmail2Pros').val() != "") {
        //     if(!isEmail($('#txtEmail2Pros').val())){
        ...
        $.ajax({
            type: 'POST',
            url: '/prospects/createLeadProspect',
            data: formData,
            dataType: 'json',
            success: function (data) {
                if (data.success) {
    ...
    $('#btnAddInterestedProduct').click(function () {
        ...
        var formData = $('#frmAddInterestedProductsProspects').serialize();
        ...
        $.ajax({
            type: 'POST',
            url: '/prospects/interestedProducts/addInterestedProduct',
    ...
    $('#updateLeadProspect').click(function () {
        ...
        /*  if($('#country').val().trim() == ""){
         ...
         if($('#businessAddress1').val().trim() == ""){
             alert("Etner Business Address 1")
             return false;
         }
         if($('#city').val().trim() == ""){
             alert("Enter City")
        ...
        if (validateForm('business-info')) {
            ...
            $.ajax({
                type: 'POST',
                url: '/prospects/details/profile/updateLeadProspect',
                data: formData,
                dataType: 'json',
                success: function (data) {
                    if (data.success) {
    ...
    $(document).on('submit', 'form', function (event) {
        ...
        if (frmName.includes("frmComment")) {
            ...
            $.ajax({
                type: 'POST',
                url: '/prospects/details/profile/addComment',
        ...
        if (frmName.includes("frmSubComment")) {
            ...
            $.ajax({
                type: 'POST',
                url: '/prospects/details/profile/addSubComment',
                data: $('#' + frmName).serialize(),
                dataType: 'json',
                success: function (data) {
                    if (data.success) {
    ...
    $('#updateContact').click(function () {
        ...
        if (validateForm('contact-person')) {
            ...
            $.ajax({
                type: 'POST',
                url: '/prospects/details/contact/updateContact',
                data: formData,
                dataType: 'json',
                success: function (data) {
                    if (data.success) {
    ...
    $('#btnProcessConvert').click(function () {
        ...
        $.ajax({
            type: 'POST',
            url: '/prospects/details/profile/convertToMerchant',
            data: formData,
            dataType: 'json',
            success: function (data) {
                if (data.success) {
    ...
    $('#frmUploadCSV').submit(function () {
        ...
        $.ajax({
            url: "/prospects/uploadfile", // Url to which the request is send
    ...
    $('#frmProspectOrder').submit(function () {
        var details = [];
        var product_id;
        var frequency;
        var qty;
        ...
        showLoadingModal("Creating Order then Converting to Merchant..... Please wait.....");
    });
    ...
    $(".btnSaveAsDraft").click(function() {
        ...
        $.ajax({
            ...
            success: function(data) {
                closeLoading();
                if (data.success) {
                    showSuccessMessage(data.message, '/prospects');
                } else {
...
function deleteProduct(prod_id) {
    if (confirm('Are you sure you want to remove?')) {
        ...
        $.ajax({
            type: 'GET',
            url: '/prospects/interestedProducts/deleteInterestedProduct',
            data: formData,
            dataType: 'json',
            success: function (data) {
                if (data.success) {
                    ...
                    // $.ajax({
                    //     type:'GET',
                    //     url:'/prospects/interestedProducts/getInterestedProducts',
                    //     data:formData,
                    //     dataType:'json',
                    //     success:function(data){
...
function updateIncomingLeadRequest(id, status) {
    ...
    if (confirm("Are you sure you want to " + status_text + " this request?")) {
        ...
        $.ajax({
            type: 'GET',
            url: '/prospects/incoming/updateIncomingLeadRequest',
            data: data,
            dataType: 'json',
            success: function (data) {
                if (data.success) {
...
function advanceSearchLeadsProspects() {
    ...
    $('#prospects-table').dataTable().fnDestroy();
    if (canviewupline == '1') {
        $('#prospects-table').DataTable({
            "lengthMenu": [25, 50, 75, 100 ],
            processing: true,
            serverSide: true,
            ajax: '/prospects/advance_leads_prospects_search/prospects/' + products,
            columns: [
                /* {
                    data: 'partner_type'
                }, */
...
function deleteProspect(id, status) {
    if (confirm('Are you sure you want to delete Prospect?')) {
        var data = {
            id: id,
            status: status
        };
...
function validateForm(curActiveHref) {
    ...
    if (curActiveHref.match('business-info')) {
        ...
        /* if ($('#txtEmailPros').val().trim() == "") {
            ...
            document.getElementById('txtEmailPros').style.removeProperty('border');
            $('#txtEmailPros-error').remove();
        } */
        if ($('#txtEmailPros').val() != "") {
            if (!isEmail($('#txtEmailPros').val())) {
                var id = "txtEmailPros";
        ...
        if ($('#isUpdate').val() == '1') {
            if ($('#mobileNumber').val().trim() == "" &&
                ($('#txtEmailPros').val().trim() == "")) {
                var id = "txtEmailPros";
                var msg = "Prospect must have either Business Email or Contact Person's Mobile Number.";
                errors[id] = msg;
            } else {
                document.getElementById('txtEmailPros').style.removeProperty('border');
                $('#txtEmailPros-error small').text();
